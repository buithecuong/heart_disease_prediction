...
class HEART_DISEASE_ANALYZER:
    ...
    def create_train_multivalue_linear_model (df,names, test_size):
       ...
       # variance score: 1 means perfect prediction 
        ...
        coef_df = pandas.DataFrame(mln_model.coef_, X1.columns, columns=['Coefficient'])  
        imp_coef = coef_df.sort_values('Coefficient')
    ...
    def train_and_test(df,k, target):
        num_df=df.select_dtypes(include=['integer','float'])
        ...
        if k==0:
            ...
            rmse_two = numpy.sqrt(mse_two)
            
            avg_rmse = numpy.mean([rmse_one, rmse_two])
            print(rmse_one)
            print(rmse_two)
            ...
            print("Test target" + str(test[target]))
            print("Prediction_two:" + str(predictions))
            print(rmse_values)
            avg_rmse = numpy.mean(rmse_values)
            return avg_rmse
...
def main():
    ...
    print()
    run.create_train_multivalue_linear_model(df, names,10)
